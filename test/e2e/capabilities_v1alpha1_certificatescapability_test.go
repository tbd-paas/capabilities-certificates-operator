...
func capabilitiesv1alpha1CertificatesCapabilityChildrenFuncs(tester *E2ETest) error {
	...
	workload, err := capabilitiescertificates.ConvertWorkload(tester.workload)
	if err != nil {
		return fmt.Errorf("error in workload conversion; %w", err)
	}

	resourceObjects, err := capabilitiescertificates.Generate(*workload, nil, nil)
	if err != nil {
...
func capabilitiesv1alpha1CertificatesCapabilityNewHarness(namespace string) *E2ETest {
	return &E2ETest{
		namespace:          namespace,
		unstructured:       &unstructured.Unstructured{},
		workload:           &capabilitiesv1alpha1.CertificatesCapability{},
		sampleManifestFile: "../../config/samples/capabilities_v1alpha1_certificatescapability.yaml",
		getChildrenFunc:    capabilitiesv1alpha1CertificatesCapabilityChildrenFuncs,
		logSyntax:          "controllers.capabilities.CertificatesCapability",
	}
}

func (tester *E2ETest) capabilitiesv1alpha1CertificatesCapabilityTest(testSuite *E2EComponentTestSuite) {
...
func (testSuite *E2EComponentTestSuite) Test_capabilitiesv1alpha1CertificatesCapability() {
	tester := capabilitiesv1alpha1CertificatesCapabilityNewHarness("")
	tester.capabilitiesv1alpha1CertificatesCapabilityTest(testSuite)
}
