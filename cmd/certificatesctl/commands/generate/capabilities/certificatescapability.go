...
func NewCertificatesCapabilitySubCommand(parentCommand *cobra.Command) {
	generateCmd := &cmdgenerate.GenerateSubCommand{
		...
		GenerateFunc: GenerateCertificatesCapability, UseWorkloadManifest: true,
		WorkloadKind: "CertificatesCapability",
	}

	generateCmd.Setup()
}

// GenerateCertificatesCapability runs the logic to generate child resources for a
// CertificatesCapability workload.
func GenerateCertificatesCapability(g *cmdgenerate.GenerateSubCommand) error {
	...
	generateFuncMap := map[string]generateFunc{
		"v1alpha1": v1alpha1certificatescapability.GenerateForCLI,
